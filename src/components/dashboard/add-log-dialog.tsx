...
export function UpsertLogDialog({ open, onOpenChange, onSave, logToEdit }: UpsertLogDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[480px]">
        <DialogHeader>
          <DialogTitle className="font-headline">{logToEdit ? 'Edit Work Log' : 'Add New Work Log'}</DialogTitle>
          <DialogDescription>
            {logToEdit ? 'Update the details of your work log.' : 'Record your accomplishments and tasks. Be detailed for better review insights.'}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
                                        ...
                                        <div className="mt-2 flex flex-wrap gap-1">
                                            {field.value.map((tag) => (
                                                <Badge key={tag} variant="secondary">
                                                    {tag}
                                                    <button type="button" onClick={() => removeTag(tag)} className="ml-1 rounded-full p-0.5 text-xs hover:bg-destructive/20">
